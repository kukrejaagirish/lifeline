...
OPENAI_KEY = os.environ.get("LIFELINE_OPENAI_KEY", "")
OPENAI_MODEL = os.environ.get("LIFELINE_OPENAI_MODEL", "gpt-4o-mini")
TRIAGE_MODE = "ai" if OPENAI_KEY else "heuristic"
...
# Uses the OpenAI API when LIFELINE_OPENAI_KEY is set; otherwise (or on
...
def main():
    ...
    print(f" SMS/WhatsApp: {'LIVE (Twilio)' if NOTIFY_MODE == 'twilio' else 'simulated'}",
          flush=True)
    print(f" AI Triage   : {'LIVE (OpenAI)' if TRIAGE_MODE == 'ai' else 'offline heuristic'}",
          flush=True)
